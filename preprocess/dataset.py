...
class Dataset(object):
	...
	# else:
		...
		for i in range(batch_size):
			randix = np.random.choice(5, sample_num, replace=False)
			# randix = 1
			if sample_num == 1:
				randix = int(randix)
				# pdb.set_trace()
				...
				sampled_embeddings.append(embeddings[i, :])
			else:
	...
	def next_batch(self, batch_size, window=None, wrong_img=False, embeddings=False, labels=False):
		...
		if self._embeddings is not None and embeddings:
			...
			class_id = [self._class_id[i] + 21 for i in current_ids]
	...
	def next_batch_test(self, batch_size, start, max_captions):
		...
		for i in range(np.minimum(max_captions, 1)):
			# batch = sampled_embeddings[:, i, :]
			batch = sampled_embeddings[:]
...
class TextDataset(object):
	def __init__(self, workdir, size):
		...
		self.image_filename = '/{}images_cvs.pickle'.format(FINAL_SIZE_TO_ORIG[size])
		...
		self.embedding_filename = '/cvs_txt_emb_512.pickle'
	...
	def get_data(self, pickle_path, aug_flag=True) -> Dataset:
		...
		with open(pickle_path + '/names.pickle', 'rb') as f:
